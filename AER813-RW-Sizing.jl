...
# ╔═╡ 7322da40-a0ed-11ec-07d3-9bb98a4ca879
using Plots, PlotThemes, HTTP, Gumbo, DataInterpolations

# ╔═╡ abe951a5-5783-4a19-94b6-7e4777505448
set_theme(:juno)

# ╔═╡ d25966f3-d70b-44f9-a106-358f98a5ce14
...
# ╔═╡ 53a9c224-18dc-4546-8186-6166c87f1bfd
orbit_alts = LinRange(200,500,500)

# ╔═╡ 4d9d0f60-1762-4695-bc2f-9c801a888782
orbits = orbit_alts .+ re

# ╔═╡ 7b7fd6f8-4e3a-4df9-bb76-5312cd7b90ae
orbit_periods = (2*π / sqrt(μ)) .* orbits.^(3/2) 

# ╔═╡ fc8b3d88-e7cc-4c6d-8ba9-41406f61ae88
hw = 0.707 .* T_D .* orbit_periods ./ 4

# ╔═╡ 9edc6a1f-0b87-4e90-8784-d3e7cff8cc46
plot(
	orbit_alts, hw, 
	xlabel="Orbit Altitude (km)", 
	ylabel="Required Momentum Storage (Nms)",
	label=""
)

# ╔═╡ 9b5dd282-f9dd-4ee7-afa2-b4ea7b44a714
md"## Upper Atmosphere Modelling
- [Properties of Standard Atmoshphere](http://www.braeunig.us/space/atmos.htm)
"

# ╔═╡ 5fa148f5-251d-4f38-81df-00341e907c30
# web scraping
begin
...
# ╔═╡ 799f7821-6bcc-41f3-8f37-e3a47849034e
# parsing and extracting body
begin
	r_parsed = parsehtml(String(r.body));
	body = r_parsed.root[2];
end;

# ╔═╡ 43e25f1d-9915-4d06-8543-15e62c444c0f
row = body[2][1][4][2][1][1][22]

# ╔═╡ 0e7db535-a6f0-4242-b813-f4dce23e7d4c
row[1][1][1].text

# ╔═╡ bf15dc45-008e-4aa3-893e-971ab252c6c1
alts, densities = Vector{Float64}(), Vector{Float64}()

# ╔═╡ 5a480302-2b7c-4715-9128-0da97844eb32
for i in 8:14
	row = body[2][1][4][2][1][1][i]

	push!(alts, parse(Float64, row[1][1][1].text))
	push!(densities, parse(Float64, row[3][1][1].text))
end

# ╔═╡ 9e2172dd-0fa9-426b-b74c-b161fe93e03c
alts, densities

# ╔═╡ 3e3a87d3-c2b0-406c-979e-f1b6f692e169
# data
# alts = [200.0, 250.0, 300.0, 350.0, 400.0, 450.0, 500.0]
# densities = [2.41e-10, 5.97e-11, 1.87e-11, 6.66e-12, 2.62e-12, 1.09e-12, 4.76e-13]

# ╔═╡ 3cd629cd-7f14-454f-9a8f-6e74de388e69
density_interp = CubicSpline(densities, alts)

# ╔═╡ b4028c3d-9d31-41d0-8209-3016efbcf5d1
begin
	plot(
		alts, densities, 
		label="Data", 
	...
	plot!(orbit_alts, density_interp.(orbit_alts), label="Cubic Spline")
end
